...
class Solution{
  public:
    ...
    int grandchil(Node *root , map<Node * , int>&mp)
    {
        int sum =0;
        if(root->left)
    ...
    int getMax(Node *root , map<Node *, int>&mp)
    {
        if(!root) return 0;
        if(mp.find(root)!=mp.end())
            return mp[root];
        int incl = root->data + grandchil(root, mp);
        int excl = getMax(root->left , mp) + getMax(root->right ,mp);
